...
async fn get_block_time(client: &mut CompactTxStreamerClient<Channel>, height: u32) -> Result<u32> {
...
mod tests {
    ...
    async fn connect_times_out_when_server_stalls_after_tcp_accept() {
        ...
        let connect_fut = tokio::spawn(connect(format!("https://127.0.0.1:{port}").leak()));
    ...
    fn mock_find_block_height(chain: &[(u32, u32)], timestamp: u32) -> u32 {
        ...
        let get_time = |h: u32| -> u32 { chain.iter().find(|(height, _)| *height == h).unwrap().1 };
